...
function showOneThumbnail(imgno) {
   ...
   filename = parent.getthfilename(imgno-1);
   pageno   = parent.getpagenumber(imgno);
   source   = parent.getsource(imgno);
   pagetype = parent.getpagetype(imgno);
   filepath= '/proj/adsset/scancheck/figs/scans/'+filename;
   imgkwds = 'bg=%%23ffffff&db_key=AST&bits=3&scale=8' ;
   imgcgi  = 'http://adsset.cfa.harvard.edu/imgs/scans/'+filename;
 
   bckg = bckgs[pagetype];

   document.write("<a href=\"javascript:loadImage("+imgno+");\"><img src=\""+imgcgi+"\" border=\"1\"></a>");
...
function loadImage(imgn) {
   ...
   //
    ...
    filename = parent.getfilename(imgn-1);
